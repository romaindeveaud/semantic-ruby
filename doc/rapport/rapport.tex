...
\par GoogleCode permet, à l'instar de SourceForge, de gérer les versions d'un projet. Il propose plusieurs fonctionnalités, telle que des issues qui permettent de signaler les problèmes à régler ou les choses à faire et de les attribuer à une personne ainsi que d'y associer une priorité. Il permet aussi de pouvoir naviguer dans le code source parmis les différentes versions ainsi que de comparer directement les modifications d'un document entre différentes version.
GoogleCode utilise SubVersion.
\par Nous avons fait le choix d'utiliser GoogleCode plutôt que SourceForge, Trac ou encore Redmine car il n'est pas nécessaire d'installer quoi que ce soit pour l'utiliser (il suffit de disposer d'un compte google), de plus la création d'un projet ce fait en quelques secondes. De plus notre choix s'étant porté sur svn car il est l'un des plus connus gestionnaire de version et que nous esperions que nous pourrions le faire faire installer à la fac, certains gestionnaire comme Redmine était exclus.
