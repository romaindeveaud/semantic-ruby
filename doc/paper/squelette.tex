...
\par L'interprétation de requêtes écrites en langage naturel par un système de recherche d'information (RI) ou de dialogue est un enjeu important de l'ingénierie documentaire. Outre que la requête en language naturelle rend possible la formulation d'une requête de recherche selon une forme bien plus précise que la simple saisie de mots clés. Dans le cadre de la compréhension des languages parlés et des systèmes de dialogue, elle permet de déployer des composants de compréhension (SLU) conviviaux pour les utilisateurs. 
...
\par Nous décrivons dans notre introduction les grands principes de la litérature sur l'analyse de requêtes en langue naturelle, et son déploiement dans le cadre de systèmes de Question Réponses (SQR). Puis nous présentons les principaux systèmes de QR existants, et les méthodes de transformations de requêtes sémantiques et d'interrogation qu'ils mettent en oeuvre. Nous expliquons également dans cette section la relation existant entre ontologies issues de corpus encyclopédiques, et le rôle que ces ontologies peuvent jouer dans des SQR. 
...
\section{Architecture du système NLGbAse.QR}\label{details}
...
\par Comme nous l'avons expliqué, les différents moteurs de recherche de NLGbAse n'attendent pas les mêmes entrées, nous allons donc détailler ici les deux types d'extraction de mots-clés - ou mots pertinents.
 \subsubsection{Moteurs de recherche d'information par similarité cosinus}
...
\subsubsection{Moteur de recherche d'information par algorithme de compacité (question-réponse)}
...
\begin{table}[h]
    \begin{center}
        \begin{tabular}{|p{2.5cm}|l|l|l|}
            ...
            Catégorie & (\={p}) & (\={r}) & (\={F}-s) \\
            \hline
            Pers & 0.75 & 0.88 & \textbf{0.83} \\
            \hline
            Org & 0.33 & 0.14 & \textbf{0.20} \\
            \hline
            Loc & 0.71 & 0.59 & \textbf{0.65} \\
            \hline
            Date & 0.89 & 0.89 & \textbf{0.89} \\
            \hline
            Amount & 0.78 & 0.63 & \textbf{0.70} \\
            \hline
            Unk & 0.56 & 0.63 & \textbf{0.59} \\
            \hline
            \hline
            Total & 0.68 & 0.63 & \textbf{0.64} \\
            \hline
        \end{tabular}
