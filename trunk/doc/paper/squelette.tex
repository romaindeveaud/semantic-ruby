...
\par Parfois cette méthode ne donne aucun résultat. Il y a plusieurs explications possibles à cela. La première est qu'il n'y a pas sur Wikipédia de fiche sur ce sujet, la seconde est que notre a mal extrait le nom propre et soumet donc quelque chose de faux à NLGbAse. La troisième raison est imputable à l'utilisateur. En effet si l'utilisateur commet une faute dans l'écriture du nom alors NLGbAse ne pourra pas nous donner la bonne réponse. Nous avons essayé de résoudre ce dernier point. La première solution serait d'avoir une base de tout les noms propres possibles et d'essayer de voir si un nom se rapproche de celui saisi par l'utilisateur mais cette solution serait très couteuse en espace de stockage. Nous avons alors pensé à utiliser Google pour essayer de corriger les fautes de ce genre. Lorsque le NLGbAse ne renvoi aucun résultat on effectue une requête avec le nom propre sur Google, ensuite nous regardons si Google suggère une autre orthographe par l'intermédiaire du "Did you mean :". Si Google suggère quelque chose alors on envoi cette suggestion à NLGbAse. Cependant cette méthode n'est pas infaillible est ne corrigera pas l'erreur si elle se retrouve beaucoup sur le net.
\par Enfin si à ce stade le nom n'est toujours pas associé à une étiquette alors nous soumettons la phrase au module de CCG de reconnaissance d'entité nommée basé sur l'architecture SNOW~\cite{ratinov:paper}. En effet ce module à de très bonnes performances pour reconnaitre les personnes, organisations et produits.
...
Notre choix s'est porté sur cette année là car à partir de l'année suivante le format des fichiers a changé et est devenu à notre sens beaucoup trop spécifique à la tâche de question réponse.
En effet les corpus les plus récents propose des séries de cinq questions en rapport avec une cible donnée. On voit bien là que l'approche est toute autre. En effet pour le moteur de ri classique avec lequel nous travaillons la meilleure manière d'obtenir un document comportant la réponse est de choisir la fiche correspondant à la cible. L'évaluation de notre application sur ces corpus n'aurait eu que très peu de sens. De plus nous trouvions qu'une évaluation de ce type est vraiment superficielle et ne reflète pas les résultats que le système fournirait à un utilisateur qui ne fournirait pas la cible de la question. Dans cette même optique de coller au plus près aux conditions réelles d'utilisation de l'outils nous avons décidé de ne pas tenir compte des informations données sur le type de questions (factoid, list , definition ...).
Nous avons donc choisi le corpus le plus récent qui correspondait à l'évaluation que nous comptions faire.
...
\par Ces résultats nous ont permis de comparer notre système avec certains des meilleurs systèmes actuels. Force est de constater que nous sommes nettement en dessous de leurs résultats comme par exemple le système de CCG ~\cite{learning:paper} qui obtient un score de 90 à 95\% de bonne classification. Cependant les méthodes utilisées par ces systèmes sont basé sur des méthodes d'apprentissage automatique. Avant même le début de notre application nous étions conscient que de tels systèmes permettaient d'obtenir des résultats meilleurs que ceux obtenu avec des méthodes et des règles faites manuellement. Cependant notre propos est plutôt de démontrer qu'il est possible d'obtenir des résultats interessants en utilisant un faible nombre de règles et de ressources ainsi que des algorithmes simples.
\par
De plus notre méthode est facilement adaptable à d'autres langues à condition que Wordnet la supporte (ou a moins de trouver des ressources identiques dans la langue voulue), d'avoir un parser dans cette langue et de traduire les quelques règles dans la langue cible. Nous obtenons alors dans ce cas des résultats à peine plus faible que le système multilingue de Thamar Solorio et al. ~\cite{solorio:paper} utilisant des heuristiques complexes et ayant recours aux svm.
\par
Enfin les résultats sont impactés par le fait que cette application dépends de plusieurs outils tel que NLGbAse qui est encore en développement et qui commet encore des erreurs que nous répétons, et tel que l'analyseur syntaxique linkgrammar qui bien qu'ayant l'avantage d'être libre n'est pas le plus performant existant et qui commet aussi des erreurs (par exemple sur l'objet de la question qu'il associe frequemment au pronom interrogatif).
